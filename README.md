...
1. **New Resource → Docker / Dockerfile**, pointing at this repository.
2. Coolify builds from the included `Dockerfile` (multi-stage, non-root).
3. **Environment variables**: set `COURT_COUNT` (e.g. `4`). `PORT` defaults to
   `3000`; `DB_PATH` defaults to `/app/data/komet.db`.
4. **Persistent volume**: mount a volume at `/app/data` so match state survives
   restarts/redeploys.
5. **Port / proxy**: expose container port `3000`. Set the domain
   (e.g. `stream.bmkkomet.se`); Coolify provisions HTTPS via its reverse proxy.
   WebSocket upgrades are proxied automatically.
6. **Health check**: the image defines a `HEALTHCHECK` against `/healthz`.

### Build & run locally with Docker

```bash
docker build -t komet-streaming .
docker run -p 3000:3000 -e COURT_COUNT=4 -v komet_data:/app/data komet-streaming
```
